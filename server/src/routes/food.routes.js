...
router.patch("/:id/reserve", async (req, res) => {
  try {
    ...
    if (food.status !== "available")
      return res.status(400).json({ message: "Food already reserved or picked up" });
    ...
    if (recipient?.email)
      await sendEmail(
        ...
        `<p>Hi ${recipient.name || "Recipient"},</p>
         <p>You have successfully reserved <b>${food.name}</b> from ${donor?.name || "a donor"}.</p>
         <p>Please pick it up before <b>${new Date(food.expiryDate).toLocaleString()}</b>.</p>
    ...
    if (donor?.email)
      await sendEmail(
        ...
        `<p>Hi ${donor.name || "Donor"},</p>
         <p>Your donation <b>${food.name}</b> has been reserved by ${recipient?.name || "a recipient"}.</p>
...
router.patch("/:id/pickup", async (req, res) => {
  try {
    ...
    if (food.reservedBy !== recipientId)
      return res.status(403).json({ message: "You cannot confirm pickup for this item." });
    ...
    if (donor?.email)
      await sendEmail(
        ...
        "Pickup Completed ✅",
        `<p>Hi ${donor.name || "Donor"},</p>
         <p>Your food item <b>${food.name}</b> has been successfully picked up.</p>
    ...
    if (recipient?.email)
      await sendEmail(
        ...
        "Pickup Verified 🍴",
        `<p>Hi ${recipient.name || "Recipient"},</p>
         <p>Your pickup of <b>${food.name}</b> has been verified successfully (${distance.toFixed(
           1
         )}m from location).</p>
